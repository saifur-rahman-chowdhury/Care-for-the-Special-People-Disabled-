...
public class DoctorPage extends javax.swing.JFrame {
    ...
    public DoctorPage() {
        ...
        doctorfield.setBackground(new Color(0,0,0,0));
        
        doctorfield.setOpaque(false);
         jTable1.setOpaque(false);
            ((DefaultTableCellRenderer)jTable1.getDefaultRenderer(String.class)).setOpaque(false);
            
        doctorfield.getViewport().setOpaque(false);
...
//sets column width
    ...
    public ArrayList<DoctorInfo> doctorList(){
        ArrayList<DoctorInfo> doctorsList = new ArrayList<>();
         conn = DatabaseHandler.connectDatabase1();
         try{
             String query = "SELECT first_name,degree,hospital,department,email FROM doctor";
             Statement st = conn.createStatement();
             ResultSet rs = st.executeQuery(query);
             DoctorInfo info;
             while(rs.next()){
                 info = new DoctorInfo(rs.getString("first_name"),rs.getString("degree"),rs.getString("hospital"),rs.getString("department"),rs.getString("email"));
    ...
    public void show_info(){
        ArrayList<DoctorInfo> list = doctorList();
        DefaultTableModel model  = (DefaultTableModel)jTable1.getModel();
        Object[] row = new Object[6];
        for(int i=0;i<list.size();i++){
            row[0] = list.get(i).getname();
            row[1] = list.get(i).getdegree();
            row[2] = list.get(i).gethospital();
